...
async function handleCartMenu(
  ...
  if (digits || speechResult) {
    const input = normalizeInput(digits, speechResult, confidence, CART_INTENTS);
      ...
      case 'checkout':
        res.type('text/xml').send(
          buildSay(
            ...
            `/api/twilio/voice/gather?step=checkout_address_choice&user_id=${userId}&call_sid=${callSid}`
          )
        );
        return;
      case 'change_item':
  ...
  res.type('text/xml').send(
    buildGather({
      ...
      inputType: 'dtmf speech',
      timeout: 10,
      hints: ['list items', 'checkout', 'change item', 'remove item', 'main menu'],
