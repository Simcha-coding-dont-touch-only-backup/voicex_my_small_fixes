  ...
  return (
    <div className="space-y-3 rounded-lg border border-indigo-200 bg-indigo-50/30 p-4">
      <div>
        ...
        <div
          className={`${IFIELD_HOST_CLASS}${cardFieldError ? ' border-red-400' : ''}`}
        >
          <IField
            ref={cardRef as any}
            type={CARD_TYPE}
            ...
            onUpdate={(data) => {
              ...
              if (issuer) setCardIssuer(issuer);
...
export function UserDetailPage() {
  ...
  const handleSaveCard = async (payload: CardSavePayload) => {
    setSavingCard(true);
    try {
      await apiPost(`/users/${id}/payment-methods`, payload);
      setAddingCard(false);
